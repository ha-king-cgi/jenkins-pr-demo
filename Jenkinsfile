node {
	...
	stage 'Print Environment'
        ...
        stage 'deploy ephemeral stack'
            sh 'author=$(git --no-pager show -s --format="%an"); aws cloudformation create-stack --stack-name Temp-$(date +%s)-Jenkins-PoC-$author --tags Key=AuthorName,Value=$author --template-body file://Jenkins-Demo-PR.json'
