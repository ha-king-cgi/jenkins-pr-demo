    ...
    switch(env.BRANCH_NAME) {
      ...
      case ~/^PR-[0-9]+/:
        ...
        stage 'Find Old Stacks'
          ...
          sh 'for stacks in $(aws cloudformation list-stacks --stack-status-filter CREATE_COMPLETE --query "StackSummaries[].StackName" | grep "Jenkins-[A-Z]*-[0-9]*-[0-9]*"  | sed s/\"//g | rev | cut -c2 3- | rev | sed s/\ //g); do time=$(echo $stacks | cut -d'/' -f2 | cut -d'-' -f4); thirty_minutes_ago=$(date -d "30 min ago" +%s);if [ "$time" -lt "$thirty_minutes_ago" ]; then aws cloudformation delete-stack --stack-name ${stacks};fi;done'
